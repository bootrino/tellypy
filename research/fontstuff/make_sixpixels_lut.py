...
result = []
lut_index_int = 0
for as_int in range(256):
        # https://stackoverflow.com/a/16660046/627492
        bits = [False for i in range(8)]
        for i in range(8):
            bits[7 - i] = True if as_int & 1 else False
            as_int = as_int >> 1

        out = ['ffff' if x else 'bbbb' for x in bits]

        pixel_string = f"""0x{out[0]}{out[1]},0x{out[2]}{out[3]},0x{out[4]}{out[5]},"""
        if lut_index_int not in lut.keys():
            lut[lut_index_int] = pixel_string
        else:
            if lut[lut_index_int] != pixel_string:
                print('values different')
                sys.exit()
        lut_index_int += 1

print("uint32_t six_pixels_lut[] = {")
lut = OrderedDict(sorted(lut.items()))
for v in lut.values():
    print(v)
print("}")
