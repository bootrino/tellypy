...
for line in data.splitlines():
    ...
    try:
        foo = foo[0].strip()
        if not foo.startswith('0x'):
            continue
        # convert the hex to a number
        # the ,0 argument to the int function makes it interpret the leading 0x
        as_int = int(foo.strip(), 0)
...
for offset in range(8):
  for i in range(offset, len(result), 8):
      final_result.append(result[i])

# until memory issues can be resolved we only have room for 64 characters
