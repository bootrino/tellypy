...
// useful information on overscan
// https://electronics.stackexchange.com/questions/305316/horizontal-overscan-on-ntsc
...
extern uint8_t terminal_ram[TERM_WIDTH * TERM_HEIGHT];
...
LOCAL void slc_isr(void) {
        ...
        if (slc_intr_status & SLC_RX_EOF_INT_ST) {
                ...
                {
                        // adjust this for overscan originally 30
                        uint8_t pixel_column = 32;

                        /* Determines which terminal line we are currently rendering,
                        192 pixel lines >> 3 = TERM_HEIGHT text lines.
                        Each is TERM_WIDTH bytes long. */

                        // the >> 3 is a divide by 8 using bit shifting
                        // if you divide the pixel lines (192) byt the height of a character (8)
                        // then you get the number of rows (24)
                        // this pointer points to the first byte of the terminal RAM
                        // presumably this byte is an integer that is an index to the font
                        uint8_t *terminal_line = &terminal_ram[TERM_WIDTH * (current_pixel_line >> 3)];

                        /* 8 banks (one for each char line) of 192 bytes contain character definitions.
                        Each character is pre-encoded to optimize performance. */

                        // ANDing the current pixel line with b111 returns a value between 0 and 7
                        // which is used as an offset into the ROM byte array
                        // this determines which slice of the font we are on
                        // current_pixel_line & 0b111
